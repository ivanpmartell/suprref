...
helper_class = Helper()
helper_class.add_variable("model", "models/tfbs_check/results/model.pt")
helper_class.add_variable("binary", False)
helper_class.add_variable("kernel_size", 7)
helper_class.add_variable("num_filters", 2)
helper_class.add_variable("stride", 7)
...
def analyse_weights(title, helper_class, save_location, filter_weights, aggregate_function, weights1=None, weights2=None):
    print(title)
    ...
    for position in range(conv_result_length):
        ...
        for idx, w in enumerate(filter_weights):
            ...
            file_location = os.path.join(save_location,"position{0}_filter{1}.svg".format(position, idx))
...
analyse_weights(title, helper_class, save_location, filter_weights, Helper.aggregate_weights, weights1=non_promoter_weights)
...
analyse_weights(title, helper_class, save_location, filter_weights, Helper.aggregate_weights, promoter_weights, non_promoter_weights)
...
analyse_weights(title, helper_class, save_location, filter_weights, Helper.aggregate_weights, non_promoter_weights, promoter_weights)

title = "Analyzing promoter weights"
save_location = os.path.join(model_folder,"seqlogo","promoter-only")
analyse_weights(title, helper_class, save_location, filter_weights, Helper.aggregate_weights, weights1=promoter_weights)